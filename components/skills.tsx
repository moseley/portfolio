        ...
        <div className="flex flex-wrap gap-3">
          {tools.map((t) => (
            <div key={t.name} className="group relative">
              <div className="h-12 w-12 sm:h-14 sm:w-14 rounded-xl border backdrop-blur-sm flex items-center justify-center transition-colors border-(--border) bg-(--surface) hover:bg-(--surface-hover) hover:border-(--border-strong)">
              ...
              <div
                role="tooltip"
                className="pointer-events-none absolute left-1/2 -translate-x-1/2 top-full mt-2 whitespace-nowrap px-2 py-1 rounded-md text-xs font-mono border opacity-0 group-hover:opacity-100 transition-opacity z-10 bg-(--tooltip-bg) text-white border-(--tooltip-border)"
              >
                {t.name}
              </div>
            </div>
